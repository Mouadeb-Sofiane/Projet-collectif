...
const activeTab = ref<"reportages" | "info">("reportages");
const showModal = ref(false);

// Propriété calculée pour gérer miniaturePlaylist
const computedMiniaturePlaylist = computed(() => {
  const mini = props.playlist.miniaturePlaylist;
  if (typeof mini === "string") {
    return mini;
  } else if (mini instanceof File) {
    return URL.createObjectURL(mini);
  } else {
    return "src/img/default/imgdedefault.jpg";
  }
});
  ...
  >
    ...
    >
      <div
        ...
        :class="{
          'opacity-100 translate-y-0': showModal,
          'opacity-0 translate-y-4': !showModal,
        }"
      >
        <!-- Header avec bannière -->
        <div class="w-full h-[400px] md:h-[500px] relative">
          <img
            :src="reportages[0] ? `https://img.youtube.com/vi/${reportages[0].videoId}/sddefault.jpg` : playlist.thumbnailurl"
            :alt="playlist.title"
            class="w-full h-full object-cover md:rounded-t-lg"
          />
          <div
            class="absolute inset-0 bg-gradient-to-t from-[#141414] via-[#141414]/60 to-transparent md:rounded-t-lg"
          ></div>
              ...
              {{
                playlist.description.length < 200
                  ...
                  : playlist.description.slice(0, 200) ||
                    "Description de l'émission non disponible"
              }}
            </p>
          </div>
        ...
        <nav class="border-b border-white/10">
          <div class="flex space-x-8 px-8 py-4">
            <button
              @click="activeTab = 'reportages'"
              :class="[
                'font-medium transition-colors',
                activeTab === 'reportages' ? 'text-white' : 'text-white/50'
              ]"
            >
              REPORTAGES
            </button>
        ...
        <div class="px-8 py-6">
          <!-- Reportages -->
          <div v-if="activeTab === 'reportages'" class="space-y-6">
            <div
            ...
            >
              ...
              <div class="flex-grow space-y-2">
                <div class="flex items-center gap-3">
                  <h3 class="font-medium text-2xl">Reportage {{ index + 1 }}</h3>
                  <span class="text-sm text-gray-400">{{ reportage.duree || "49 min" }}</span>
                </div>
                <h4
                  class="text-lg font-medium group-hover:text-primaryColor transition-colors"
                >
                  {{ reportage.title }}
                </h4>
                <p class="text-gray-400 text-sm">
                  {{
                    reportage.description.length > 400
                      ? reportage.description.slice(0, 400) + "..."
          ...
          <div v-if="activeTab === 'info'" class="max-w-3xl mx-auto space-y-8">
            ...
            <div class="grid grid-cols-2 gap-8">
              ...
              <div>
                ...
                <ul class="space-y-2 text-gray-300">
                  <li>
                    <span class="text-gray-500">Réalisation:</span> IUT Nord
                    Franche-Comté
                  </li>
                  <li>
                    <span class="text-gray-500">Production:</span> Service
                    Communication
                  </li>
                  <li>
                    <span class="text-gray-500">Distribution:</span> Web TV IUT
                  </li>
                </ul>
              </div>
            </div>
